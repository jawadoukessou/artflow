  ...
  async getPredictions(companyId: string) {
    const customers = await this.prisma.customer.findMany({
      ...
      include: {
        ...
        invoices: { where: { status: { in: ['OVERDUE','PARTIALLY_PAID'] } } },
      },
      take: 20,
    });
    return customers.map(c => ({
      customerId: c.id,
      name: c.name,
      outstanding: c.invoices.reduce((s,i) => s + Number(i.amountDue), 0),
      riskScore: c.riskScores[0]?.score || 0,
      riskLevel: c.riskScores[0]?.level || 'LOW',
      lateProbability: Math.min(95, (c.riskScores[0]?.score || 0) * 0.9 + Math.random() * 10),
      recommendation: this.getRecommendation(c.riskScores[0]?.level),
    })).filter(p => p.outstanding > 0).sort((a,b) => b.lateProbability - a.lateProbability);
  }
